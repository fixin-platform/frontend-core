...
Template.connection.onRendered ->
  ...
  $reconnect = $(@firstNode)
  if not $reconnect.data('bs.popover')
#    Not Using `destroyed` callback now
    $(".connection-popover").remove() # cleanup old popovers
    $reconnect.on("shown.bs.popover", ->
      $(@).data('bs.popover').$tip.addClass("connection-popover")
    )
    $reconnect.popover(
      html: true
...
#Template.connection.onDestroyed ->
#  clearTimeout(hideTimeout)
#  $reconnect = $(@firstNode)
#  $reconnect.popover("destroy")
